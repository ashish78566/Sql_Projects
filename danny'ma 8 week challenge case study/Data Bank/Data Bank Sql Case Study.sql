...
VALUES
 ...
 select count(distinct node_id) as node_per_region
 from customer_nodes
 group by region_id
 order by region_id;
 
 #3.How many customers are allocated to each region?
 select r.region_name, count(distinct c.customer_id) as customer_per_region
...
select * from customer_transactions;
select txn_type, count(distinct customer_id) as unique_customers,
...
SELECT ROUND(AVG(DATEDIFF(end_date, start_date)), 2) AS avg_days_to_reallocation
FROM customer_nodes
...
ordered AS (
    SELECT
        region_id,
        days,
        ROW_NUMBER() OVER (PARTITION BY region_id ORDER BY days) AS rn,
...
SELECT
    ...
    MAX(CASE WHEN rn = FLOOR((total_count + 1) / 2) THEN days END) AS median_days,
    -- 80th percentile
    MAX(CASE WHEN rn = CEIL(0.8 * total_count) THEN days END) AS p80_days,
    -- 95th percentile
    MAX(CASE WHEN rn = CEIL(0.95 * total_count) THEN days END) AS p95_days
FROM ordered
GROUP BY region_id
ORDER BY region_id;
...
select avg(deposite_count) as avg_deposite_count,avg(deposite_amount) as avg_deposite_amount from (
select customer_id,count(*) as deposite_count,sum(txn_amount) as deposite_amount
from customer_transactions 
where txn_type='deposit'
group by customer_id) as customer_deposite;
...
or sum(case when txn_type='wthdrawal' then 1 else 0 end) >=1) order by month;


#4.What is the closing balance for each customer at the end of the month?
select  customer_id,date_format(txn_date,'%Y-%m') as month,sum(case when txn_type='deposit' then txn_amount 
else -txn_amount end ) as closing_balance from customer_transactions
...
#Step1:Running Balance for Each Transaction
SELECT
    customer_id,
    txn_date,
    txn_type,
...
#Balance at the End of Each Month (Option 1)
...
#Option 2: data is allocated on the average amount of money kept in the account in the previous 30 days
select customer_id,txn_date,avg(running_balance) over(partition by customer_id order by txn_date range
 between interval 30 day PRECEDING and current row) as avg_30d_balance 
 from (select customer_id,txn_date,sum(case when txn_type='deposit' then txn_amount else -txn_amount end)
 over(partition by customer_id order by txn_date rows between unbounded preceding and current row) as running_balance
from customer_transactions)rb;
...
#running customer balance column that includes the impact each transaction
select customer_id,txn_date,txn_type,txn_amount,sum(case when txn_type='deposit' then txn_amount else -txn_amount end)
over(partition by customer_id order by txn_date rows between unbounded preceding and current row) as running_balance
from customer_transactions
order by customer_id,txn_date;
...
SELECT
    ...
    MAX(running_balance) AS end_of_month_balance
